...
// GetReachable returns destinations reachable from a city in the FlixTrain network.
func (c *Client) GetReachable(ctx context.Context, cityQueryOrID string, limit int) ([]domain.Airport, error) {
	...
	endpoint := fmt.Sprintf("%s/cms/cities/%s/reachable?language=en-us&country=US&limit=%d&transportation_category=flixtrain,train", baseURL, cityUUID, limit)
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("flixtrain reachable error: status %d", resp.StatusCode)
	}
