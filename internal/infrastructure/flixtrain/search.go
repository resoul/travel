...
// SearchTrips searches for FlixTrain trips between two cities on a given date.
func (c *Client) SearchTrips(ctx context.Context, criteria domain.FlightSearchCriteria) ([]domain.FlightOffer, error) {
	...
	// Format to DD.MM.YYYY required by FlixTrain v4 API
	var flixDate string
	...
	endpoint := fmt.Sprintf(
		"%s/search/service/v4/search?from_city_id=%s&to_city_id=%s&departure_date=%s&products=%s&currency=%s&locale=en_US&search_by=cities&transportation_category=flixtrain,train&include_after_midnight_rides=1",
		baseURL,
		fromUUID,
		toUUID,
	...
	if err != nil {
		return nil, fmt.Errorf("failed to perform flixtrain search: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("flixtrain search error: status %d", resp.StatusCode)
	}

	var searchResp searchResponseDTO
	if err := json.NewDecoder(resp.Body).Decode(&searchResp); err != nil {
		return nil, fmt.Errorf("failed to decode flixtrain search response: %w", err)
	}
