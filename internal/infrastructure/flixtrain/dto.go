...
func (r *rideResultDTO) toDomain(currency string, cities, stations map[string]namedEntityDTO) domain.FlightOffer {
	...
	depParsed, depFormatted := parseFlixTrainTime(r.Departure.Date)
	arrParsed, arrFormatted := parseFlixTrainTime(r.Arrival.Date)
...
func parseFlixTrainTime(raw string) (*time.Time, string) {
